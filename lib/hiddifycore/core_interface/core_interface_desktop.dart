...
import 'package:melavpn/gen/hiddify_core_generated_bindings.dart';
...
final _logger = Loggy('MelaVPNCoreFFI');
...
class CoreInterfaceDesktop extends CoreInterface with InfraLogger {
  static final MelaVPNCoreNativeLibrary _box = _gen();

  static MelaVPNCoreNativeLibrary _gen() {
    String fullPath = "";
    if (Platform.environment.containsKey('FLUTTER_TEST')) {
      fullPath = "melavpn-core";
    }
    if (Platform.isWindows) {
      fullPath = p.join(fullPath, "melavpn-core.dll");
    } else if (Platform.isMacOS) {
      fullPath = p.join(fullPath, "melavpn-core.dylib");
    } else {
      fullPath = p.join(fullPath, "melavpn-core.so");
    }

    _logger.debug('melavpn-core native libs path: "$fullPath"');
    ...
    return MelaVPNCoreNativeLibrary(lib);
  }
